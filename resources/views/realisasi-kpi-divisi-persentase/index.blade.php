    ...
    <div class="container py-4">
        <div class="card">
            <div class="card-header">
                <h5 class="mb-2">Realisasi KPI Divisi - Persentase</h5>
                ...
                <div class="d-flex align-items-center justify-content-between flex-wrap gap-2">
                    ...
                    <form method="GET" action="{{ route('realisasi-kpi-divisi-persentase.index') }}"
                        ...
                        <a href="{{ route('realisasi-kpi-divisi-persentase.index', ['per_page' => $perPage]) }}"
                    ...
                    <table class="table-hover table align-middle">
                        ...
                        <tbody>
                            @if (is_null($bulan) || is_null($tahun) || (in_array($me->role, ['owner', 'hr']) && empty($division_id)))
                                <tr>
                                    <td colspan="9" class="text-muted py-4 text-center">Silakan pilih filter.</td>
                                </tr>
                            @else
                                @forelse ($kpis as $i => $k)
                                    @php
                                    ...
                                    <tr>
                                        ...
                                        <td class="text-end">
                                            @if ($r && !is_null($r->realization))
                                                {{ rtrim(rtrim(number_format($r->realization, 2, '.', ''), '0'), '.') }}
                                                {{ $k->satuan }}
                                            @else
                                                -
                                            @endif
                                                ...
                                                @if ($needsReinput)
                                                    <div class="d-flex align-items-center flex-nowrap gap-2">
                                                        <a class="btn btn-icon btn-sm btn-primary"
                                                            href="{{ route('realisasi-kpi-divisi-persentase.create', ['kpi_divisi_id' => $k->id]) }}"
                                ...
                                @empty
                                    <tr>
                                        <td colspan="9" class="text-muted py-4 text-center">Tidak ada KPI persentase
                                            pada periode ini.</td>
                                    </tr>
                                @endforelse
                            @endif
