    ...
    <div class="container py-4">
        <div class="card border-0 shadow-sm rounded">
            <div class="card-header d-flex justify-content-between align-items-center flex-wrap gap-2">
                <div>
                    <h5 class="mb-0"><i class="bx bx-message-square-check me-1"></i>Detail Realisasi KPI Divisi Response - {{ $real->user->full_name }}</h5>
                    <small class="text-muted">{{ $bulanList[$real->bulan] }} {{ $real->tahun }} -
                        {{ $real->division?->name }}</small>
                </div>
                <a href="{{ route('realisasi-kpi-divisi-response.index', ['bulan' => $real->bulan, 'tahun' => $real->tahun, 'division_id' => $real->division_id]) }}"
