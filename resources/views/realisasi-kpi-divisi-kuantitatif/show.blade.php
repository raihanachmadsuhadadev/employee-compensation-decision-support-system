                    ...
                    @if (auth()->user()->role === 'hr' && $real->status === 'submitted')
                        <div class="d-flex gap-2">
                            <form id="approveForm" method="POST"
                                action="{{ route('realisasi-kpi-divisi-kuantitatif.approve', $real->id) }}">
                                @csrf
...
@push('scripts')
    ...
    <script>
        function approveReal() {
            Swal.fire({
                title: 'ACC Realisasi?',
                text: 'Pastikan data sudah benar. Tindakan ini akan mengesahkan realisasi KPI Divisi Kuantitatif.',
                icon: 'question',
